<template>
  <el-dialog
    ...
    <div slot="title">
      ...
      <el-button v-show="ticket.state===0" class="float-right" style=" margin-left: 30px;" @click="changeTicketState(1)" type="warning" plain>بستن تیکت</el-button>
      <el-button v-show="ticket.state===1" class="float-right" style=" margin-left: 30px;"  @click="changeTicketState(0)" type="primary" plain>باز کردن تیکت</el-button>
    </div>
       ...
       <ul>
         <template v-for="item in ticket.ticketItems">
           ...
           <div class="message-data align-right">
             <span class="message-data-name">{{item.userFullName}}</span> <i class="fa fa-circle me"></i>
           ...
           <div :class="['message','other-message','float-left',(editing===item?'editing':'')]">
             ...
             <span v-show="editing!==item" class="edit float-right" @click="setEditing(item)">ویرایش <i
               class="el-icon-edit"></i></span>
             <span v-show="editing===item" class="edit float-right">درحال ویرایش ... <i
               class="el-icon-edit"></i></span>
           </div>
         </li>
           ...
           <div class="message-data align-left">
             <span class="message-data-name"><i class="fa fa-circle online"></i> {{item.userFullName}}</span>
             <span class="message-data-time">{{ item.createdOn | persian-date }}</span>
           </div>
    ...
    <el-form :inline="true" class="form" ref="form" label-width="120px">
      ...
      <textarea ref="textarea" v-model="message" @keyup.enter.exact="submit" placeholder="متن پیام" class="message-box"
                rows="5"></textarea>
    </el-form>
  </el-dialog>
</template>
    ...
    methods: {
      changeTicketState(state) {
        this.$confirm('از تغییر وضعیت این تیکت اطمینان دارید؟', 'هشدار', {
          ...
          axios.patch('/api/tickets/changestate', {
      ...
      updateEditing() {
        axios.patch('/api/tickets/ticketitems', {
      ...
      addNewAnswer() {
        axios.post('/api/tickets/ticketitems', {
          ticketId: this.ticket.id,
          message: this.message
        }).then(res => {
