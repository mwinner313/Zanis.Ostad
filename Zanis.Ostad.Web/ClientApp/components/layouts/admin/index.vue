          ...
          <router-link to="/admin/courses">
            <el-menu-item index="3">
              ...
              <span slot="title">
                دوره ها
                <el-badge
                  v-if="!!coursesOverView && coursesOverView.pendingToApproveByAdmin"
                  :value="coursesOverView.pendingToApproveByAdmin"
                />
              </span>
            </el-menu-item>
          </router-link>
...
export default {
  ...
  mounted() {
    ...
    if (!user || !user.roles.some(x => x.toLowerCase() === "admin")) {
  ...
  methods: {
    ...
    loadUnReadTicketItemsCount() {
      axios
        .get("/api/tickets", { params: { pageOffset: 0, pageSize: 1 } })
        .then(res => {
